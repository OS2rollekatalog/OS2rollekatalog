...
public class AdSyncServiceApiV2 {
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Trigger a synchronization of all AD groups"),
		@ApiResponse(responseCode = "404", description = "No client found")})
	@Operation(summary = "Receives the ADConfiguration from the Client")
	@PostMapping("/api/v2/ad/syncAll/{domain}")
	public ResponseEntity<HttpStatus> synchronizeAll(@PathVariable("domain") String argDomain) {
		final Domain domain = domainService.getDomainOrPrimaryOptional(argDomain)
			.orElseThrow(() -> new IllegalArgumentException("No domain found with name: " + argDomain));
		pendingADUpdateService.addAllGroups(domain.getId());

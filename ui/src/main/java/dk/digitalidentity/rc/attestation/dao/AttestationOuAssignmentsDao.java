...
public interface AttestationOuAssignmentsDao extends CrudRepository<AttestationOuRoleAssignment, Long> {
    ...
    @Query(value = "SELECT s FROM AttestationOuRoleAssignment s WHERE s.validFrom <= :validAt AND (s.validTo > :validAt or s.validTo is null) AND s.responsibleCollectionId is null AND s.ouUuid=:ouUuid AND s.inherited=false AND s.exceptedTitleUuids is not null AND LENGTH(s.exceptedTitleUuids) > 0")
    ...
    @Query(value = "SELECT s FROM AttestationOuRoleAssignment s WHERE s.validFrom <= :validAt AND (s.validTo > :validAt or s.validTo is null) " +
		"AND s.responsibleCollectionId in (:responsibleCollectionIds) AND s.inherited=false")
    List<AttestationOuRoleAssignment> listValidNotInheritedAssignmentsWithResponsibleCollectionIdIn(@Param("validAt") final LocalDate validAt, @Param("responsibleCollectionIds") final List<Long> responsibleCollectionIds);
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM attestation_ou_role_assignments s2 " +
            ...
            "GROUP BY s.attestation_responsible_collection_id, s.sensitive_role) as sub on sub.sid = s2.id")
    List<AttestationOuRoleAssignment> findValidGroupByResponsibleCollectionIdAndSensitiveRole(@Param("validAt") final LocalDate validAt);

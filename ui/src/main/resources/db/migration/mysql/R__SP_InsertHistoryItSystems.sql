...
BEGIN
  ...
  INSERT INTO history_it_systems (
    dato, it_system_id, it_system_name, it_system_hidden, attestation_exempt)
  SELECT CURRENT_TIMESTAMP, it.id, it.name, it.hidden, it.attestation_exempt
  FROM it_systems it;

  INSERT INTO history_it_system_attestation_responsible (history_it_systems_id, user_uuid)
  SELECT hit.id, ar.user_uuid
  FROM history_it_systems hit
  JOIN it_system_attestation_responsible ar ON ar.itsystem_id = hit.it_system_id
  WHERE hit.dato = CAST(CURRENT_TIMESTAMP AS DATE);

  INSERT INTO history_it_system_system_owner (history_it_systems_id, user_uuid)
  SELECT hit.id, so.user_uuid
  FROM history_it_systems hit
  JOIN it_system_system_owner so ON so.itsystem_id = hit.it_system_id
  WHERE hit.dato = CAST(CURRENT_TIMESTAMP AS DATE);

...
DELIMITER $$
DROP PROCEDURE IF EXISTS SP_InsertHistoryOUNegativeRoleAssignmentsOUInheritRecursive $$

CREATE PROCEDURE SP_InsertHistoryOUNegativeRoleAssignmentsOUInheritRecursive (IN ou_roles_id int, IN orig_ou_uuid VARCHAR(36), IN orig_ou_name VARCHAR(255), IN ou_roles_ou_uuid VARCHAR(36))
BEGIN
    ...
    getChild: LOOP
        ...
        Call SP_InsertHistoryOUNegativeRoleAssignmentsOUInheritRecursive(ou_roles_id, orig_ou_uuid, orig_ou_name, child_ou_uuid);
    END LOOP getChild;
...
DROP PROCEDURE IF EXISTS SP_InsertHistoryOUNegativeRoleAssignmentsOUInherit $$
CREATE PROCEDURE SP_InsertHistoryOUNegativeRoleAssignmentsOUInherit ()
BEGIN
    DECLARE finished INTEGER DEFAULT 0;
    ...
    getInherited: LOOP
        ...
        Call SP_InsertHistoryOUNegativeRoleAssignmentsOUInheritRecursive(ou_roles_id, ou_roles_ou_uuid, ou_name, ou_roles_ou_uuid);
...
DELIMITER $$
DROP PROCEDURE IF EXISTS SP_InsertHistoryOUNegRoleAssignmentsOURoleGroupInheritRecursive $$

CREATE PROCEDURE SP_InsertHistoryOUNegRoleAssignmentsOURoleGroupInheritRecursive (IN ou_roles_id int, IN orig_ou_uuid VARCHAR(36), IN orig_ou_name VARCHAR(255), IN ou_roles_ou_uuid VARCHAR(36))
BEGIN
    DECLARE finished INTEGER DEFAULT 0;
    DECLARE child_ou_uuid VARCHAR(36);
    DECLARE inherit BIT;
    ...
    getChild: LOOP
        ...
        Call SP_InsertHistoryOUNegRoleAssignmentsOURoleGroupInheritRecursive(ou_roles_id, orig_ou_uuid, orig_ou_name, child_ou_uuid);
    END LOOP getChild;
...
DELIMITER $$
DROP PROCEDURE IF EXISTS SP_InsertHistoryOUNegativeRoleAssignmentsOURoleGroupInherit $$

CREATE PROCEDURE SP_InsertHistoryOUNegativeRoleAssignmentsOURoleGroupInherit ()
BEGIN
    DECLARE finished INTEGER DEFAULT 0;
    DECLARE ou_roles_id int;
    DECLARE ou_roles_ou_uuid VARCHAR(36);
    ...
    getInherited: LOOP
        ...
        Call SP_InsertHistoryOUNegRoleAssignmentsOURoleGroupInheritRecursive(ou_roles_id, ou_roles_ou_uuid, ou_name, ou_roles_ou_uuid);
    END LOOP getInherited;
...
BEGIN
    INSERT INTO history_role_assignment_negative_titles (
        ...
        assigned_through_type, assigned_through_uuid, assigned_through_name,
        assigned_by_user_id, assigned_by_name, assigned_when, inherit, start_date, stop_date)
    SELECT CURRENT_TIMESTAMP, o.uuid, GROUP_CONCAT(DISTINCT oureu.title_uuid),
    ...
    FROM ou_roles our
             ...
             JOIN it_systems it ON it.id = ur.it_system_id
             JOIN ou_roles_titles oureu ON oureu.ou_roles_id = our.id
    WHERE our.inherit = 0 AND o.active = 1 AND our.contains_excepted_users = 0 AND our.contains_titles = 2
    GROUP BY (our.id);
    ...
    INSERT INTO history_role_assignment_negative_titles (
        ...
        assigned_through_type, assigned_through_uuid, assigned_through_name,
        assigned_by_user_id, assigned_by_name, assigned_when, inherit, start_date, stop_date)
    SELECT CURRENT_TIMESTAMP, o.uuid, GROUP_CONCAT(DISTINCT ourgeu.title_uuid),
           ur.id, ur.name, it.id, it.name, rg.name, rg.id,
           'DIRECT', NULL, NULL,
    ...
    WHERE ourg.inherit = 0 AND o.active = 1 AND ourg.contains_excepted_users = 0 AND ourg.contains_titles = 2
    GROUP BY ur.id, o.uuid, ourg.id;

    -- user roles from orgunits (inherited)
    CALL SP_InsertHistoryOUNegativeRoleAssignmentsOUInherit();

    -- user roles through rolegroups from orgunits (inherited)
    CALL SP_InsertHistoryOUNegativeRoleAssignmentsOURoleGroupInherit();
